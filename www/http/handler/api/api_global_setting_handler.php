<?php
    ...
    function handle_api_global_setting_get($uri_parts, $uri_args) {
        $repos = query('SELECT * FROM global_setting');
        echo json_encode($repos);
        exit();
    }

    function handle_api_global_setting_post($uri_parts, $uri_args) {
        $payload = $_POST ?? null;
        ...
        $check_query = 'SELECT * FROM global_setting WHERE name = :setting_name';
        ...
        $query = 'INSERT INTO global_setting (name, value) VALUES (:setting_name, :setting_value)';
    ...
    function handle_api_global_setting_put($uri_parts, $uri_args) {
        $payload = null;
        ...
        $check_result = query($check_query, $check_params);
        if($check_result) {
            $query = 'UPDATE global_setting SET value = :setting_value WHERE name = :setting_name';
            $params = array('setting_name' => $setting_name, 'setting_value' => $setting_value);
            query($query, $params);
            $new_setting = query($check_query, $check_params);
            echo json_encode($new_setting);
