<?php
    ...
    function test_repo_post_1($properties) {
        ...
        assertEquals('Provided payload is empty', '{"status":"failed","error":"No entity provided in payload"}', "$uri did not trigger error for empty payload");

        $response = test_curl($properties['endpoint_url'] . "/$uri", ['name' => 'test'], true);
        assertEquals(400, $response['http_code'], "$uri http code mismatch");
        assertEquals('Provided payload is empty', '{"status":"failed","error":"Required field `url` is not provided in the payload"}', "$uri did not trigger error for empty payload");

        $response = test_curl($properties['endpoint_url'] . "/$uri", ['url' => 'test'], true);
        assertEquals(400, $response['http_code'], "$uri http code mismatch");
        assertEquals('Provided payload is empty', '{"status":"failed","error":"Required field `name` is not provided in the payload"}', "$uri did not trigger error for empty payload");
        ...
        assertEquals(400, $response['http_code'], "$uri http code mismatch");
        assertEquals('Provided payload is empty', '{"status":"failed","error":"Required field `download_location` is not provided in the payload"}', "$uri did not trigger error for empty payload");
